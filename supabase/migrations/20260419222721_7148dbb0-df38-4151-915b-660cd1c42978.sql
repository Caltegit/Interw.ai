-- Remplace la fonction de seed avec les 6 nouveaux modèles métiers
...
BEGIN
  ...
  LOOP
    -- Skip si déjà existant (préserve les modifs utilisateur)
...
-- Supprime les anciens modèles génériques par défaut s'ils n'ont pas été modifiés (suppression conservative : seulement si le nom + description correspondent exactement aux seeds d'origine)
DELETE FROM public.interview_templates
WHERE (name, description) IN (
  ('Entretien Commercial', 'Modèle pour évaluer un profil commercial : motivation, expérience terrain, capacité à négocier.'),
  ('Entretien Développeur', 'Modèle pour évaluer un développeur : parcours technique, projets, méthode de travail.'),
  ('Entretien Manager', 'Modèle pour évaluer un profil de manager : leadership, gestion d''équipe, vision.'),
  ('Entretien Stage / Alternance', 'Modèle court pour étudiant : motivation, projet pro, soft skills.'),
  ('Entretien découverte générique', 'Modèle généraliste pour un premier échange découverte.')
);

-- Rejoue le seed pour toutes les organisations existantes
DO $$
DECLARE _org RECORD;
BEGIN
