...
BEGIN
  ...
  media_paths AS (
    SELECT
      CASE
        WHEN split_part(obj.name, '/', 2) ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
          THEN split_part(obj.name, '/', 2)::uuid
        ELSE NULL
      END AS sid,
      COALESCE((obj.metadata->>'size')::bigint, 0) AS file_size
  ...
  media AS (
    SELECT
      mp.sid,
      count(*) FILTER (WHERE mp.file_size > 1024) AS n_real
    FROM media_paths mp
    WHERE mp.sid IN (SELECT sid FROM all_sids)
    GROUP BY mp.sid
  ),
  latest_reports AS (
