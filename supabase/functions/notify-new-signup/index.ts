// Alerte interne : une personne vient de créer un compte depuis le site.
...
// Limite de débit en mémoire : 3 requêtes / 10 min par IP.
...
Deno.serve(async (req) => {
  ...
  const signedUpAt = new Intl.DateTimeFormat('fr-FR', {
    dateStyle: 'long',
    timeStyle: 'short',
  ...
  try {
    await sendAppEmail('new-signup', email, {
      idempotencyKey: `new-signup-${email}`,
      replyTo: email,
      templateData: { email, fullName, signedUpAt },
    })
    return json(200, { ok: true })
  } catch (e) {
    console.error('notify-new-signup failed', e)
